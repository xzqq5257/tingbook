...
```
tingbook/  （仓库根目录 = Cloudflare Pages 根 = 项目根）
...
> 部署前自检见 `docs/DEPLOY_CHECKLIST.md`：需设 ADMIN_USER/ADMIN_PASS、绑定 KV 命名空间、确认 `wrangler.toml` 的 `binding="TINGBOOK_KV"` 与控制台一致、F5 后端用支持 `ref_audio` 的部署包。
