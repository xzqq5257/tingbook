...
def parse_books():
    # BOOKS is a single-line JSON array: [{"title":..., "duration":N, "text":"...", "file":"audio/..."}, ...]
    html = open(INDEX, encoding="utf-8").read()
    m = re.search(r'const BOOKS\s*=\s*(\[.*?\]);', html, re.S)
    if not m:
        return []
    ...
    for b in data:
        title = b.get("title", "")
        text = b.get("text", "")
        # JSON already decoded \n into real newlines; normalise whitespace to commas
        text = re.sub(r'\s+', '，', text)
        out.append((title, text, b.get("file", "")))
    return out
